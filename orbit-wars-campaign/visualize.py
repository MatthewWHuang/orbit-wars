...
<div class="wrap">
  ...
  <div class="controls">
    ...
    <button id="cinemaBtn" style="background:#7a2b8c">Cinema</button>
    <button id="ultraBtn" style="background:#d8377c">Ultra Cinema</button>
  </div>
</div>
<script>
...
function drawGraphs() {
  ...
  for (let panel = 0; panel < 3; panel++) {
    ...
    let maxV = 1;
    for (const arr of stackedDatasets[panel]) for (const v of arr) if (v > maxV) maxV = v;
    ...
    for (let p = 0; p < N_AGENTS; p++) {
      ...
      for (let i = 0; i < arr.length; i++) {
        const x = left + (i / Math.max(1, T - 1)) * innerW;
        const y = bot - (arr[i] / maxV) * innerH;
        if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
      }
      ctx.stroke();
    }
    const px = left + (cur / Math.max(1, T - 1)) * innerW;
  ...
  const innerW = right - left, innerH = bot - top;
  let maxAbs = 1;
  for (const v of series.shipDelta) if (Math.abs(v) > maxAbs) maxAbs = Math.abs(v);
  ...
  // Filled area between line and zero. Positive (p0 ahead): COLORS[0]; negative: COLORS[1].
  // Draw two clipped passes -- one above zero, one below.
  const drawFilled = (sign) => {
    const color = sign > 0 ? COLORS[0] : COLORS[1];
    ctx.fillStyle = color + '55';
    ctx.beginPath();
    ctx.moveTo(left, zeroY);
    for (let i = 0; i < series.shipDelta.length; i++) {
...
function drawSide() {
  ...
  for (let p = 0; p < N_AGENTS; p++) {
    ...
    const reward = last.rewards[p];
    const status = last.statuses[p];
    html += `<div style="margin-bottom:10px">
      <span class="pill" style="background:${COLORS[p]};color:#000">${NAMES[p] || ('Player ' + p)}</span>
      <span style="color:#8a96b8;font-size:11px">${status || ''} reward=${reward ?? '-'}</span>
      <div class="stat-row"><span class="lbl">Ships</span><b>${ships}</b></div>
  ...
  for (let t = 1; t < T; t++) {
    ...
    for (const p of cur.planets) {
      ...
      if (before && before.owner !== o) {
        ...
        events.push({
          t, type: 'flip', pid, x: p[2], y: p[3], r: p[4],
          fromOwner: before.owner, toOwner: o, prod: p[6],
          ships: p[5], importance: 60 + p[6] * 15 + Math.min(impactShips, 50),
        });
      }
    }
...
function buildShots(selected, totalSteps) {
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    // Impact freeze + caption read time
    shots.push({
      kind: 'impact', event: e, atStep: e.t, holdFrames: 110,
      target: { cx: e.x, cy: e.y, zoom: focusZoom * 1.08 },
    });
...
function buildCinema() {
  ...
  return {
    ...
    biggestFleet: 0,
    // Ultra-mode extras
    ultra: ultraMode ? buildUltraData() : null,
...
function processStepTransition(t) {
  ...
  // Damage numbers for ship-count changes on planets.
  ...
  for (const p of cur.planets) {
    ...
    // Suppress production-only increments (small +N each turn).
...
function drawCinemaFrame() {
  ...
  if (ultraMode && newStep > cinemaState.lastSeenStepBoundary) {
  ...
  // Apply screen shake to camera target tiny offsets
  if (cinemaState.shake > 0) {
    cinemaState.shake *= 0.85;
    if (cinemaState.shake < 0.5) cinemaState.shake = 0;
  }
  ...
  for (const p of fb.planets) {
    ...
    const isComet = (fa.comet_planet_ids || []).includes(p[0]);
    drawPlanetTexture(bctx, px + sx, py + sy, sr, p[1], isComet, p[6]);
    // ship count
    bctx.fillStyle = '#fff';
    const fs = Math.max(9, sr * 0.65);
    bctx.font = `bold ${fs}px sans-serif`;
    bctx.textAlign = 'center'; bctx.textBaseline = 'middle';
    bctx.fillText(String(p[5]), px + sx, py + sy);
  }
  ...
  // Ultra: fleet trails -- record current interpolated positions and draw fading tail.
  if (ultraMode) {
  ...
  // Ultra: comet trajectory preview -- dashed faint line forward from each comet.
  if (ultraMode && cinemaState.ultra) {
  ...
  // Ultra: record-fleet tags floating on the actual fleet (instead of a
  // full-screen caption that stacks and obstructs the view).
  if (ultraMode && cinemaState.recordFleetTags && cinemaState.recordFleetTags.length) {
  ...
  // Ultra: contested planet badges + damage numbers
  if (ultraMode && cinemaState.ultra) {
  ...
  // Ultra: mid-action stat callout during slowmo/impact shots
  if (ultraMode) {
    const curShot = cinemaState.shots[cinemaState.shotIdx];
  ...
  // Ultra: rich match progress timeline just below the slim bar.
  if (ultraMode) {
    const tlY = 8, tlH = 14;
...
cinemaBtn.onclick = () => {
  if (cinemaMode) stopCinema(); else { ultraMode = false; startCinema(); }
};
ultraBtn.onclick = () => {
  if (cinemaMode) {
    // Rebuild state in place -- avoid the stop/start race that causes the
    // deferred regular draw() to paint over the new cinema canvas.
    ultraMode = true;
    cinemaState = buildCinema();
    const s0 = cinemaState.shots[0];
    if (s0?.target) {
...
// Auto-start if URL has #cinema/#ultra or ?cinema=1/?ultra=1
const autoUltra = location.hash.includes('ultra') ||
  new URLSearchParams(location.search).get('ultra') === '1';
const autoCinema = autoUltra ||
  location.hash.includes('cinema') ||
  new URLSearchParams(location.search).get('cinema') === '1';

buildTabs();
loadMatch(0);
resizeCanvases();
if (autoCinema) { ultraMode = autoUltra; startCinema(); }
</script>
</body></html>
"""
